...
if [[ ! -f "${REBOTARM_MUJOCO_MODEL_DIR}/rs_arm.xml" ]] ||
   [[ ! -f "${REBOTARM_MUJOCO_MODEL_DIR}/meshes/base_link.STL" ]]; then
  echo "Integrated RS MuJoCo model is missing: ${REBOTARM_MUJOCO_MODEL_DIR}" >&2
  exit 1
fi
